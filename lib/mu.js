...
mu.fs = function (filename, callback) {
  filename = filename.indexOf('.') < 0?(filename + '.mustache'):filename;
 ...
 * @throws {Error(template_not_in_cache)} If filename was not found in cache.
 */
mu.renderSync = function (filenameOrParsed, view) {
  var parsed = typeof filenameOrParsed === 'object' ?
                  filenameOrParsed :
                  mu.cache[filenameOrParsed];
  ...
  if (parsed) {
    parsed = parsed[0] || parsed;
    return beginRenderSync(parsed.tokens, view, mu.cache);
  } else {
    throw new Error('template_not_in_cache'); //errors.templateNotInCache(filename)));
  }
}
...
function beginRenderWithStream(tokens, view, partials, stream) {
  ...
  nextTick(function () {
    try {
      renderer.render(tokens, prepareView(view), partials, stream, function () {
        stream.emit('end');
      });
    } catch (err) {
...
function beginRenderSync(tokens, view, partials) {
  return renderer.renderSync(tokens, prepareView(view), partials);
}
...
mu.renderFile = function (templateName, view, callback) {
  var stream = new Stream();
  var parsed = mu.cache[templateName];
...
function prepareView(view) {
  ...
  if (!Array.isArray(view)) {
    resView = [mu.globals, view];
  } else {
    resView=[];
    resView.push(mu.globals);
