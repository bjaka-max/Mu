...
function renderSync(render, contextData, renders) {
  var context = new Context(null, contextData, renders);
...
exports.tokenRenders.Section.prototype = {
  process: function(context, next) {
    ...
    if (res!==false && res!==null && res!==undefined && res!==0 && res!=="" && res===res) {
    ...
    if (res!==false && res!==null && res!==undefined && res!==0 && res!=="" && res===res) {
      if (res instanceof Array) {
        res.forEach(function (val) {
          context.pushValLevel(val);
          text += self.render.processSync(context);
          context.popValLevel();
        });
      } else if (typeof res === 'object') {
        context.pushValLevel(res);
...
exports.tokenRenders.ParentPartial.prototype = {
  ...
  processSync: function(context) {
    context.pushValLevel();
    this.render.processSync(context);
